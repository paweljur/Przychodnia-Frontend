...
export class UserServiceProxy {
  ...
  putUser(id: number, user: User): Observable<FileResponse | null> {
    let url_ = this.baseUrl + '/api/user/{id}';
    if (id === undefined || id === null) throw new Error("The parameter 'id' must be defined.");
    url_ = url_.replace('{id}', encodeURIComponent('' + id));
    url_ = url_.replace(/[?&]$/, '');
  ...
  authenticate(credentials: LoginCredentialsDto): Observable<FileResponse | null> {
...
export class LoginCredentialsDto implements ILoginCredentialsDto {
  ...
  constructor(data?: ILoginCredentialsDto) {
    if (data) {
      for (var property in data) {
        if (data.hasOwnProperty(property)) (<any>this)[property] = (<any>data)[property];
      }
  ...
  static fromJS(data: any): LoginCredentialsDto {
    data = typeof data === 'object' ? data : {};
    let result = new LoginCredentialsDto();
    result.init(data);
    return result;
  }
